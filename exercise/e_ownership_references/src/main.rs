...
fn main() {
    ...
    //    prints whether the contents of the String is plural or singular. Then uncomment and run
    //    this code with `cargo run apple` and `cargo run apples'.  Hint: use `.ends_with("s")` on
    ...
    //    `cargo run apple`.  Hint: use `.push_str("s")` on the mutable String reference to add an
    //    "s".
    change(&mut arg);
